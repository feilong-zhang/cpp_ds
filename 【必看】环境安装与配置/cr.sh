...
c++ "$sourceName.cpp" -o "$sourceName" -O2 --std=c++20 -DDEBUG -Wall -fdiagnostics-color=always 2>&1 | tee _compile.log
# 编译失败
...
if [ $# -ge 2 ]; then
  ...
  if [ "$(echo "$sourceName" | cut -c1)" = "/" ]; then
    cat "$2" | $sourceName
  else
    cat "$2" | ./$sourceName
  fi
