...
### MSYS2 官网 下载 msys2
[https://www.msys2.org/]
> 软件安装到 50% 会卡主，不用管，十分钟后会自动结束
### 在 msys2 ucrt64 中下载 gcc:
...
> C:\msys64\ucrt64\bin
6. 一路确定
7. VSCode 需要重新启动来应用新的环境变量，如果还不生效，可以重启电脑

## maxOS 安装 gcc
先在终端通过这条语句安装 homebrew
`  /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"  `
此时，通过 `c++ --version` 应该可以正常运行 c++ 了
但是使用的是 clang 编译器而不是 gcc，和 gcc 有细微差别，大部分情况下没太大区别
嫌麻烦的同学或者初学者可以跳过下面这段，直接使用 clang

想要正统的 gcc，则需要再通过 homebrew 直接安装 gcc
`  brew install gcc   `
然后 c++ 的编译指令就需要从 `c++` 改成 `c++-14` (如果后续版本有升级，可能会变成 c++-15，注意看安装时的信息)
后续配置 cr 文件时，可以把内部的编译指令也改成 `c++-14`
...
cr.ps1 保存到某个目录
将目录地址添加到环境变量
> 同样，需要重启一下 vscode 新的环境变量才会生效，如果不生效，需要重启电脑
...
将 cr.sh 保存到某个文件夹，如用户目录下新建的 script 文件夹
> 整个路径上最好不要有空格（不然下面配置 alias 配置的时候，需要在空格前加上一个 \ 来进行转移）
在 vscode 中打开【用户】目录
查看是否有这些文件 `.zshrc` 或 `.bashrc` 文件
> 根据你使用的 shell 来决定使用哪个文件，可以看 vscode 终端界面的右侧，是 zsh 还是 bash
如果没有，可以在用户目录下新建 `.zshrc`
在 rc 文件中添加
`  alias cr="sh {替换为你的 cr.sh 所存放目录, 连同{}一起}/cr.sh"  `
如
`  alias cr="sh ~/script/cr.sh"  `
保存后重启 vscode 就可以使用
...
## files.autoSave
修改成 onFocusChange，可以实现【自动保存】，这个根据自己诉求看要不要加